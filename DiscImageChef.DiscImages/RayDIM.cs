...
namespace DiscImageChef.ImagePlugins
{
	public class RayDIM : ImagePlugin
	{
		...
		readonly string DimSignatureRegEx = "Disk IMage VER (?<major>\\d).(?<minor>\\d) Copyright \\(C\\) (?<year>\\d{4}) Ray Arachelian, All Rights Reserved\\.";
		...
		public override bool IdentifyImage(Filter imageFilter)
		{
			...
			DicConsole.DebugWriteLine("Ray Arachelian's Disk IMage plugin", "header.sectorsPerTrack = {0}", header.sectorsPerTrack);

			Regex sx = new Regex(DimSignatureRegEx);
			Match sm = sx.Match(signature);

			DicConsole.DebugWriteLine("Ray Arachelian's Disk IMage plugin", "header.signature matches? = {0}", sm.Success);
		...
		public override bool OpenImage(Filter imageFilter)
		{
			...
			ImageInfo.imageApplicationVersion = string.Format("{0}.{1}", sm.Groups["major"].Value, sm.Groups["minor"].Value);
