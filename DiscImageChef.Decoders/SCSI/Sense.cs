...
namespace DiscImageChef.Decoders.SCSI
{
    ...
    public static class Sense
    {
        ...
        public static DescriptorSense? DecodeDescriptor(byte[] sense, out string senseDescription)
        {
            ...
            while(offset < sense.Length)
                if(offset + 2 < sense.Length)
                {
                    ...
                    if(!decoded.Descriptors.ContainsKey(descType)) decoded.Descriptors.Add(descType, desc);
        ...
        public static string PrettifySense(FixedSense? sense)
        {
            ...
            if(decoded.SegmentNumber > 0) sb.AppendFormat("On segment {0}", decoded.SegmentNumber).AppendLine();
            if(decoded.Filemark) sb.AppendLine("Filemark or setmark found");
            if(decoded.EOM) sb.AppendLine("End-of-medium/partition found");
            if(decoded.ILI) sb.AppendLine("Incorrect length indicator");
            if(decoded.InformationValid) sb.AppendFormat("On logical block {0}", decoded.Information).AppendLine();
