...
public sealed class ArchiveSubdirectory : UserControl
{
    ...
    {
        if(DataContext is not ArchiveSubdirectoryViewModel vm || sender is not DataGrid grid) return;

        vm.SelectedEntries.Clear();
        vm.SelectedEntries.AddRange(grid.SelectedItems.OfType<ArchiveFileModel>());
    }
}